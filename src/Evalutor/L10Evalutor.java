...
public class L10Evalutor {
	 ...
	 * @param dataSet
	 */
	public L10Evalutor(PredictorInterface pre, DataReader dr,NeighbourMap neighbour, SimilarityMap simMap){
		this.dr=dr;
		...
		for(Iterator<Map.Entry<UserItemPair, Double>> it=dr.getDataSet().entrySet().iterator();it.hasNext();){
			...
			Double predictRating=pre.GetPredictRating(pair.getUserID().intValue(), pair.getItemID().intValue(), dr, neighbour, simMap);
	...
	public double getL10Coverage(){
		...
		System.out.println(count_predict);
